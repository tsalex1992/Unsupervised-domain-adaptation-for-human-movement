...
weight_name = os.path.join(dirname, 'models/pytorch_Realtime/model/pose_model.pth')
...
def handle_one(oriImg):
   ...
   # paf = nn.UpsamplingBilinear2d((oriImg.shape[0], oriImg.shape[1])).cuda()(output1)
    ...
    return heatmap  #for now we have assumed that we will cut the function here and we will rely on this output
...
if __name__ == "__main__":
 ...
 #       ret, frame = video_capture.read()
    frame = cv2.imread('./sample_image/ski.jpg')
    m_heatmap = handle_one(frame)
